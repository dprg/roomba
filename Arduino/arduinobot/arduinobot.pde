...
{ 
  ...
  Wire.send('A');         // Send a "Post Heading Data" (0x50) command to the HMC6352  
  ...
  { 
    heading = Wire.receive();  // receive high byte (overwrites previous reading) 
...
{
  Wire.beginTransmission(compassAddress);  // transmit to device
  Wire.send('C');         // Send the calibration command 
  Wire.endTransmission();  // stop transmitting 
  ...
  while (!Serial.available())  // loop till user presses a key
    ;
  Wire.beginTransmission(compassAddress);  // transmit to device
  Wire.send('E');         // Send the calibration command 
  Wire.endTransmission();  // stop transmitting 
}
...
{ 
  ...
  Wire.beginTransmission(compassAddress);  // transmit to device
  Wire.send('r');         // Send a "Read eeprom/ram" command to the HMC6352
  Wire.send(addr);       // send the address to read
  Wire.endTransmission();  // stop transmitting 
...
{ 
  ...
  Wire.beginTransmission(compassAddress);  // transmit to device
  Wire.send('w');         // Send a "Write eeprom/ram" (0xf1) command to the HMC6343  
  Wire.send(addr);       // send the address to read
  Wire.send(writeData);
  Wire.endTransmission();  // stop transmitting 
  ...
  Wire.beginTransmission(compassAddress);  // transmit to device
  Wire.send('r');         // Send a "Read eeprom/ram" (0xe1) command to the HMC6343  
  Wire.send(addr);       // send the address to read
  Wire.endTransmission();  // stop transmitting 
