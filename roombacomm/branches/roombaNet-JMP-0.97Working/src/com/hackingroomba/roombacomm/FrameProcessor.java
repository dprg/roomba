...
public class FrameProcessor {
	...
	public void displayFrame()
	{
		...
		img = frame.createImage(new MemoryImageSource(imgWidth,imgHeight,vidBufInt, 0, imgWidth));
	...
	public int readFrame(int width, int height)
	{
		frameSize = width * height;
		int readLength = 0;
		int maxReadSize = frameSize;
		vidBuf = new byte[frameSize];			// analysis form - only low byte set
		vidBufInt = new int[frameSize];		// display version (alpha set, bytes replicated)
		
		vidBuf[0] = (byte)200;	// send the "capture" command
		byte[] t = new byte[4];
		
		try {
			out.write(vidBuf, 0, 1);
			out.flush();
			readLength = in.read(t, 0, 4); // read imgWidth
			imgWidth = (int)(t[2] & 0xff) << 8 | t[3] & 0xff;
			readLength = in.read(t, 0, 4); // read imgHeight
			imgHeight = (int)(t[2] & 0xff) << 8 | t[3] & 0xff;
			readLength = in.read(t, 0, 4); // read imgLength
			frameSize = (int)(t[2] & 0xff) << 8 | t[3] & 0xff;

			//System.out.println("Read image size " + imgWidth + "x" + imgHeight + " length " + frameSize);
			
			// ACHTUNG - vidBuf gets overwritten at the beginning by multiple read buffers - use vidBufInt for image
			int vidBufIx = 0;
			for (int i=0; i<5; i++) {
				readLength = in.read(vidBuf, 0, maxReadSize);
				maxReadSize -= readLength;
				for (int j=0; j<readLength; j++) {
					vidBufInt[vidBufIx++] = (255 << 24) | (int)(vidBuf[j] & 0xff) << 16 | (int)(vidBuf[j] & 0xff) << 8 | (vidBuf[j] & 0xff);
				}
				if (vidBufIx == frameSize)
					break;
				try {
					Thread.sleep(10);
	 ...
	 * camera looking at white before this is called. It processes vidBufInt at the requested rows.
	 */
	public double [] normalizeRows(int startRow, int endRow)
	{
		...
		for (int i=start; i<end; i++) {
			normalizeArray[i] = 192.0 / (double)(vidBufInt[start + i]);
		}
	...
	// take a start & end offset into the vidBuf array
	private int quantize(int start, int end) { 
		...
		for (int srcIx=start; srcIx<end; srcIx++, x++) {
			qDisplayBuf[x] = ((vidBufInt[srcIx] & 0xff) < threshold) ? 0xff<<24 : -1;	// assign quantized values to each pixel
			q[x] = ((vidBufInt[srcIx] & 0xff) < threshold) ? 0 : 1;	// assign quantized values to each pixel
		}
		return(0);
	}
...
//		System.out.println();
	...
	void dumpVideo(int row)
	{
		...
		for (int col=0; col<160; col++) {
			System.out.print((int)vidBuf[row*160 + col] + " ");
		}
		System.out.println();
	}
